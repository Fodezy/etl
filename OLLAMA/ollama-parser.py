...
def call_gemma3_27b(prompt: str) -> dict:
    """
    Sends the full prompt to Google Gemma 3 27B (free) on OpenRouter
    and returns a dict with the same shape as ollama.chat response.
    """
    ...
    # ─── DEBUG DUMP ───────────────────────────────────────────────
    print("🛠️  [DEBUG] Raw OpenRouter JSON response:")
    print(json.dumps(data, indent=2))
    # match ollama.chat’s return shape
    return {"message": {"content": data["choices"][0]["message"]["content"]}}
# In ollama_parser.py, locate the _simplify_logical_container function
...
def parse_prerequisite(prereq_string: str, prompt_template: str) -> RequisiteExpression | None:
    ...
    simplified_string, placeholder_map = pre_parse_and_replace(prereq_string)

    final_string_for_llm = simplified_string if placeholder_map else prereq_string

    filled_prompt = prompt_template.replace("{prerequisite_string}", final_string_for_llm).strip()
    
    if placeholder_map:
        mapping_json = json.dumps(placeholder_map, indent=2)
    ...
    for attempt in range(1, MAX_ATTEMPTS + 1):
        ...
        try:
            ...
            # response = ollama.chat(...)  # ← old, stubbed out

            # FREE API ENDPOINT 
            response = call_gemma3_27b(filled_prompt)
            ...
            resolved = substitute_placeholders(data_dict, placeholder_map)
            validated = RequisiteExpression.model_validate(resolved)
        ...
        except (ValidationError, json.JSONDecodeError, ollama.ResponseError, Exception) as e:
